...
## Current Status
- **Phase**: v0.1.1 Complete (P1-P4 done)
...
## P4 Ship v0.1.1 (2026) — Done
- Parallel search + caches + strict filtering + MCP compliance (published crates.io, gh release, pushed)
- Version string fix (--version/--help read CARGO_PKG_VERSION), published + released
- README: MCP client configs (Claude Code, pi), --no-filter flag, honest latency section (cold ~2s floor, warm ~0.3s), 120ms claim corrected to ~1s API floor
- Binary refreshed at ~/.local/bin/jev-scout.exe (v0.1.1) — `jev-scout` on PATH is current
- Registry listings (smithery/glama/modelcontextprotocol): manual form/PR submission, not automated from CLI; documented MCP setup in README covers manual install path
